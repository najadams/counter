    ...
    <DialogContent showCloseButton={false} initialFocus={nameRef} className="w-[min(42rem,calc(100%-2rem))] max-h-[94vh] gap-0 p-0">
      ...
      <div className="flex-1 overflow-y-auto px-6 py-5 flex flex-col gap-5">
        <section className="grid sm:grid-cols-2 gap-4">
          <Field label="Customer name"><Input ref={nameRef} value={displayName} onChange={(e) => setDisplayName(e.target.value)} /></Field>
          <Field label="Primary phone" hint={canEditCreditPolicy ? 'A unique login/search identifier.' : 'Supervisor or above can change this.'}>
            <Input className="font-mono" value={phone} disabled={!canEditCreditPolicy} onChange={(e) => setPhone(e.target.value)} />
          </Field>
          <Field label="Alternate phone"><Input className="font-mono" value={alternatePhone} onChange={(e) => setAlternatePhone(e.target.value)} placeholder="Optional" /></Field>
          <Field label="Business name"><Input value={businessName} onChange={(e) => setBusinessName(e.target.value)} placeholder="Optional" /></Field>
          <Field label="Location description"><Input value={locationDescription} onChange={(e) => setLocationDescription(e.target.value)} placeholder="Landmark or delivery area" /></Field>
          <Field label="Preferred sales channel">
            <NativeSelect value={preferredChannel} onChange={(e) => setPreferredChannel(e.target.value as typeof preferredChannel)}>
        ...
        <section className="subpanel p-4 flex flex-col gap-4">
          ...
          <div className="grid sm:grid-cols-3 gap-4">
            <Field label="Customer type"><NativeSelect value={customerType} disabled={!canEditCreditPolicy} onChange={(e) => setCustomerType(e.target.value as typeof customerType)}>{CUSTOMER_TYPES.map((type) => <option key={type.code} value={type.code}>{type.label}</option>)}</NativeSelect></Field>
            <Field label="Credit limit (cedis)"><Input className="font-mono tnum" value={creditLimit} disabled={!canEditCreditPolicy} onChange={(e) => setCreditLimit(e.target.value)} /></Field>
            <Field label="Terms (days)"><Input className="font-mono tnum" inputMode="numeric" value={creditTermsDays} disabled={!canEditCreditPolicy} onChange={(e) => setCreditTermsDays(e.target.value.replace(/\D/g, ''))} /></Field>
          </div>
        ...
        <Field label="Notes"><Textarea rows={4} maxLength={1000} value={notes} onChange={(e) => setNotes(e.target.value)} placeholder="Delivery instructions, contact context, or account notes" /></Field>
      </div>
